...
export class FilingsService {
  ...
  async assignAdvisor(
    ...
    if (!advisor) {
      throw new BadRequestException(
        'Invalid advisor — user not found or not a TAX_ADVISOR',
      );
    }
  ...
  async updateFilingDetails(
    ...
    if (dto.totalIncome !== undefined)
      updateData.totalIncome = new Prisma.Decimal(dto.totalIncome);
    if (dto.taxPayable !== undefined)
      updateData.taxPayable = new Prisma.Decimal(dto.taxPayable);
    if (dto.taxPaid !== undefined)
      updateData.taxPaid = new Prisma.Decimal(dto.taxPaid);
    if (dto.refundAmount !== undefined)
      updateData.refundAmount = new Prisma.Decimal(dto.refundAmount);
