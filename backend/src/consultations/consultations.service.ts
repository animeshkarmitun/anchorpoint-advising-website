...
export class ConsultationsService {
  ...
  async getAvailableSlots(date: string) {
    ...
    for (const schedule of schedules) {
      ...
      for (
        let m = startMinutes;
        m + schedule.slotDuration <= endMinutes;
  ...
  async bookConsultation(clientId: string, dto: BookConsultationDto) {
    ...
    if (!advisorId) {
      ...
      advisorId = advisors.sort(
        (a, b) =>
          a._count.consultationsAsAdvisor - b._count.consultationsAsAdvisor,
      )[0].id;
    }
    ...
    const consultation = await this.prisma.consultation.create({
      data: {
        ...
        meetingLink:
          dto.medium === 'VIDEO'
            ? `https://meet.google.com/placeholder-${Date.now()}`
            : null,
      },
      include: {
        advisor: {
  ...
  async rescheduleConsultation(
    ...
    if (!consultation) {
      throw new NotFoundException(
        'Consultation not found or cannot be rescheduled',
      );
    }
    ...
    if (hoursUntil < 2) {
      throw new BadRequestException(
        'Cannot reschedule less than 2 hours before the appointment',
      );
    }
  ...
  async cancelConsultation(
    ...
    if (!consultation) {
      throw new NotFoundException(
        'Consultation not found or cannot be cancelled',
      );
    }
