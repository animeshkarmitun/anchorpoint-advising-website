...
export class DocumentsService {
    ...
    if (dto.filingId) {
      ...
      if (!filing) {
        throw new BadRequestException(
          'Filing not found or does not belong to you',
        );
      }
    }
  ...
  async getChecklist(userId: string, filingId: string) {
    ...
    const requiredCategories =
      checklistMap[filing.serviceType] || checklistMap.individual;
    ...
    return {
      ...
      data: {
        ...
        completionRate: Math.round(
          (checklist.filter((c) => c.status === 'ACCEPTED').length /
            checklist.length) *
            100,
        ),
      },
  ...
  async reviewDocument(
    ...
    if (
      ['REJECTED', 'NEEDS_REUPLOAD'].includes(dto.status) &&
      !dto.rejectionNote
    ) {
      throw new BadRequestException(
  ...
  async requestDocument(dto: RequestDocumentDto, requesterId: string) {
    ...
    await this.prisma.notification.create({
      data: {
        ...
        link: dto.filingId
          ? `/filings/${dto.filingId}/documents`
          : '/documents',
      },
    });
