...
export class PaymentsService {
  ...
  async processRefund(
    ...
    if (dto.status === 'approved') {
      ...
      const isPartial =
        dto.amount && dto.amount < Number(refund.payment.amount);
  ...
  async getRevenueStats() {
    ...
    const [monthRevenue, yearRevenue, total, byMethod, byStatus] =
      await Promise.all([
        this.prisma.payment.aggregate({
  ...
  async recordManualBkashPayment(
    dto: import('./dto/manual-bkash.dto').ManualBkashPaymentDto,
  ) {
    ...
    return {
      success: true,
      message:
        'Payment recorded. We will verify and contact you within 12 hours.',
      data: {
        id: payment.id,
